...
function normalizeLessonMedia(l={}){const items=Array.isArray(l.mediaItems)?l.mediaItems.map(x=>({type:['image','video','audio','link'].includes(x?.type)?x.type:'link',url:cleanMediaUrl(x?.url),caption:String(x?.caption||'').trim()})).filter(x=>x.url):[];if(items.length)return items;return [['image',l.imageUrl],['video',l.videoUrl],['audio',l.audioUrl]].filter(([,u])=>cleanMediaUrl(u)).map(([type,url])=>({type,url:cleanMediaUrl(url),caption:''}))}
function lessonMediaHtml(l){const items=normalizeLessonMedia(l);if(!items.length)return '';const cards=items.map((m,i)=>{const u=m.url,cap=escMedia(m.caption||''),yt=m.type==='video'?youtubeEmbedUrl(u):'';if(m.type==='image')return `<figure class="lesson-media-item"><img src="${escMedia(driveMediaUrl(u,'image'))}" alt="${cap||`ภาพประกอบ ${escMedia(l.title)}`}" loading="lazy"><figcaption>🖼️ ${cap||`ภาพประกอบ ${i+1}`}</figcaption></figure>`;if(m.type==='video')return `<div class="lesson-media-item"><b>🎬 ${cap||`วิดีโอ ${i+1}`}</b>${yt?`<iframe src="${escMedia(yt)}" title="${escMedia(l.title)} video ${i+1}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`:`<video controls playsinline preload="metadata" src="${escMedia(driveMediaUrl(u,'file'))}"></video>`}</div>`;if(m.type==='audio')return `<div class="lesson-media-item"><b>🔊 ${cap||`เสียงประกอบ ${i+1}`}</b><audio controls preload="metadata" src="${escMedia(driveMediaUrl(u,'file'))}"></audio></div>`;return `<div class="lesson-media-item"><b>🔗 ${cap||`แหล่งเรียนรู้เพิ่มเติม ${i+1}`}</b><a class="secondary" href="${escMedia(u)}" target="_blank" rel="noopener noreferrer">เปิดลิงก์ ↗</a></div>`}).join('');return `<section class="card lesson-media-card"><div class="lesson-block-guide-head"><div><h3>🎨 สื่อประกอบบทเรียน</h3><p>มีสื่อทั้งหมด ${items.length} รายการ สามารถเลื่อนดูได้ตามลำดับที่ครูกำหนด</p></div></div><div class="lesson-media-grid">${cards}</div></section>`}
