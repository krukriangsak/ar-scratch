...
class HandTracker {
  ...
  setScrollEnabled(enabled) {
    this.scrollEnabled = Boolean(enabled);
    if (!this.scrollEnabled) {
      this.lastScrollY = null;
      this.scrollActive = false;
      this.elements.cursorElement?.classList.remove("scrolling");
  ...
  processResults(results) {
    ...
    const twoFingerScroll = this.scrollEnabled && indexTip.y < indexPip.y && middleTip.y < middlePip.y && Math.abs(indexTip.y - middleTip.y) < 0.09 && !isPinching;
    ...
    if (isPinching && !this.pinching) {
      const dragTarget = document.elementFromPoint(this.cursor.x, this.cursor.y)?.closest('[data-hand-draggable="true"]');
      if (dragTarget) {
        this.beginHandDrag(dragTarget, this.cursor.x, this.cursor.y);
      } else if (Date.now() - this.lastClickAt > clickCooldown) {
        this.lastClickAt = Date.now();
        this.clickAt(this.cursor.x, this.cursor.y);
      }
    } else if (isPinching && this.dragState) {
      this.updateHandDrag(this.cursor.x, this.cursor.y);
    } else if (!isPinching && this.pinching && this.dragState) {
      this.endHandDrag(this.cursor.x, this.cursor.y);
    }
  ...
  beginHandDrag(source, x, y) {
    ...
    ghost.style.width = `${Math.max(140, Math.min(source.getBoundingClientRect().width, 280))}px`;
    document.body.appendChild(ghost);
    this.dragState = { source, ghost };
    source.classList.add('hand-drag-source');
    this.updateHandDrag(x, y);
    source.dispatchEvent(new CustomEvent('handdragstart', { bubbles: true, detail: { x, y, source } }));
  }
  ...
  updateHandDrag(x, y) {
    ...
    document.querySelectorAll('[data-hand-dropzone="true"].hand-drop-hover').forEach(el => el.classList.remove('hand-drop-hover'));
    const target = document.elementFromPoint(x, y)?.closest('[data-hand-dropzone="true"]');
    target?.classList.add('hand-drop-hover');
  }
  ...
  endHandDrag(x, y) {
    ...
    ghost.remove();
    source.classList.remove('hand-drag-source');
    document.querySelectorAll('[data-hand-dropzone="true"].hand-drop-hover').forEach(el => el.classList.remove('hand-drop-hover'));
    const target = document.elementFromPoint(x, y)?.closest('[data-hand-dropzone="true"]');
    if (target) {
      target.dispatchEvent(new CustomEvent('handdrop', { bubbles: true, detail: { x, y, source } }));
    }
    source.dispatchEvent(new CustomEvent('handdragend', { bubbles: true, detail: { x, y, source, target } }));
    this.dragState = null;
  }
  ...
  stop() {
    ...
    if (this.dragState) {
      ...
      this.dragState.source?.classList.remove('hand-drag-source');
      this.dragState = null;
    }
